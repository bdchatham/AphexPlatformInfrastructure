...
create_or_select_cluster() {
  if [[ "$USE_EXISTING" == true ]]; then
    ...
    if kind get clusters 2>/dev/null | grep -q "^${CLUSTER_NAME}$"; then
      ...
      kind create cluster --name "$CLUSTER_NAME" --config platform/deployments/kind/bootstrap/kind-cluster-config.yaml
...
create_root_application() {
  ...
  kubectl apply -f platform/base/argocd/apps/platform-root.yaml
